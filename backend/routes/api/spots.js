...
router.get("/current", requireAuth, async (req, res) => {
  ...
  for (let i = 0; i < findSpot.length; i++) {
    ...
    findSpot[i].dataValues.avgRating = parseFloat(
...
router.post(
  ...
  async (req, res) => {
    ...
    bookingDate.forEach((booking) => {
      ...
      if (
        (booking.toJSON().startDate <= new Date(endDate) &&
          booking.toJSON().endDate >= new Date(endDate)) ||
        (booking.toJSON().startDate <= new Date(startDate) &&
                        ...
                        // }if (lng) {
                            ...
                            // }if (name) {
                                ...
                                // }if (description) {
                                    ...
                                    // console.log('update_backend',updateSpot)
...
router.get("/", validatePage, async (req, res) => {
  ...
  for (let i = 0; i < getAllspot.length; i++) {
    ...
    getAllspot[i].dataValues.avgRating = Number(
      countRating[0].dataValues.avgRating
    ).toFixed(1);
